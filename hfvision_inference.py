...
from DeepDataMiningLearning.hfvisionmain import load_visionmodel, load_dataset, dataset_preprocessing
...
class MyVisionInference():
    ...
    def convert_to_torchscript(self):
        self.model = torch.jit.trace(self.model, self.inputs)
        self.model = torch.jit.optimize_for_inference(self.model)
        print("Model converted to TorchScript")

    def train_model(self, dataset, num_epochs=10):
    ...
    def load_new_dataset(self, dataset_name="detection-datasets/coco", split="train", cache_dir=None):
        dataset = load_dataset(dataset_name, split=split, cache_dir=cache_dir, trust_remote_code=True)
        return dataset

    def evaluate_coco(self, dataset):
        val_loader = DataLoader(dataset["val"], batch_size=8, shuffle=False)
        coco_evaluator = evaluate.load("coco")

        for images, targets in val_loader:
            images = images.to(self.device)
            outputs = self.model(images)
            coco_evaluator.add_batch(predictions=outputs, references=targets)
...
def object_detection(mycache_dir=None):
    ...
    myinference.convert_to_torchscript()
...
def main():
    myinference = MyVisionInference(model_name="facebook/detr-resnet-50", task="object-detection")

    dataset = myinference.load_new_dataset("detection-datasets/coco", split=None)  # Load entire dataset
    print(dataset)
    print(dataset['train'][0])
    print("Available dataset splits:", dataset.keys())

    dataset = myinference.load_new_dataset("detection-datasets/coco", split='train')

    myinference.train_model(dataset, num_epochs=10)

    # before TorchScript conversion
    test_image_path = "../sampledata/bus.jpg"
    print("Inference before TorchScript conversion:")
    results = myinference(test_image_path)

    myinference.convert_to_torchscript()

    # after TorchScript conversion
    print("Inference after TorchScript conversion:")
    results = myinference(test_image_path)

    myinference.evaluate_coco(dataset)

    # MyVisionInferencetest(task="object-detection", mycache_dir=None) # for inference speed test (no training)
